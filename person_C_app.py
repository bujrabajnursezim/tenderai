...
st.set_page_config(
    page_title="TenderAI — Аудитор тендеров",
    page_icon="🔍",
    layout="wide"
)
...
.logo { font-size: 2.2rem; font-weight: 700; color: #6C63FF; letter-spacing: -1px; }
.tagline { color: #888; font-size: 0.9rem; margin-top: -8px; }
...
def highlight_text(text, suspicious_sentences):
    RESTRICTION_WORDS = ["строго", "исключительно", "авторизованный дилер",
                         "уполномоченный дилер", "эксклюзивный", "только данного",
                         "аналоги не принимаются", "не допускается замена"]
    snippet = text[:3000]
...
def generate_pdf_report(result, filename):
    ...
    try:
        ...
        if score >= 70:
            ...
            pdf.set_text_color(0, 204, 102)
        pdf.cell(0, 10, f"Risk Score: {score}/100  |  {result.get('level', '')}", ln=True, align="C")
        pdf.ln(4)
        pdf.set_font("Helvetica", "", 11)
        pdf.set_text_color(50, 50, 80)
        verdict = result.get("verdict", "")
        if verdict:
            pdf.multi_cell(0, 7, f"Verdict: {verdict}")
        pdf.ln(4)
        pdf.set_font("Helvetica", "B", 12)
        pdf.set_text_color(108, 99, 255)
        pdf.cell(0, 8, "AI Explanations:", ln=True)
        pdf.set_font("Helvetica", "", 10)
        pdf.set_text_color(50, 50, 80)
        for exp in result.get("explanations", []):
            text_clean = f"- {exp.get('text', '')}".encode('latin-1', 'replace').decode('latin-1')
        ...
        if rec:
            ...
            pdf.set_font("Helvetica", "", 10)
            pdf.set_text_color(50, 50, 80)
            pdf.multi_cell(0, 6, rec.encode('latin-1', 'replace').decode('latin-1'))
        return pdf.output(dest='S').encode('latin-1')
    except Exception:
        return None

# ─── ЗАГОЛОВОК ───────────────────────────────────────────
st.markdown('<div class="logo">⚡ TenderAI</div>', unsafe_allow_html=True)
st.markdown('<div class="tagline">AI-аудитор госзакупок Казахстана</div>', unsafe_allow_html=True)
st.markdown("---")

tab1, tab2, tab3 = st.tabs(["🔍 Анализ тендера", "⚖️ Сравнить два тендера", "📋 История анализов"])

# ══ ВКЛАДКА 1 ══
with tab1:
    uploaded_file = st.file_uploader("Загрузите тендерную документацию", type=["pdf", "docx"], key="main_upload")
    ...
    if uploaded_file:
        ...
        if len(text) < 50:
            ...
            if FPDF_AVAILABLE:
                pdf_bytes = generate_pdf_report(result, uploaded_file.name)
                if pdf_bytes:
                    st.download_button(
                        label="📥 Скачать PDF отчёт",
                        data=pdf_bytes,
                        file_name=f"TenderAI_report_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf",
                        mime="application/pdf"
                    )
            else:
                st.info("Для скачивания PDF установите: `python -m pip install fpdf2`")
...
with tab2:
    ...
    if file1 and file2:
        ...
        if diff > 0:
            st.markdown(f'<div class="exp-high">🚨 <b>{file1.name}</b> более рискованный тендер (Risk Score: {r1["risk_score"]}/100)</div>', unsafe_allow_html=True)
        elif diff < 0:
            st.markdown(f'<div class="exp-high">🚨 <b>{file2.name}</b> более рискованный тендер (Risk Score: {r2["risk_score"]}/100)</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="exp-medium">⚖️ Оба тендера имеют одинаковый уровень риска</div>', unsafe_allow_html=True)
...
with tab3:
    ...
    if not st.session_state.history:
        st.markdown('<p style="color:#ccc;text-align:center;margin-top:2rem;">История пуста — загрузите тендер для анализа</p>', unsafe_allow_html=True)
    else:
        for item in st.session_state.history:
            ...
            <div class="{css}">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <div>
                        <span style="font-weight:600;font-size:0.9rem;">{icon} {item['name']}</span>
...
st.markdown('<p style="text-align:center;color:#eee;font-size:0.7rem;margin-top:3rem;">TENDERAI // HACKATHON 2026 // KAZAKHSTAN</p>', unsafe_allow_html=True)